...
import {
    ...
    Trash2, Search, ChevronRight, Lock
} from 'lucide-react'
...
export default function CompaniasPage() {
    ...
    const fetchData = async () => {
        ...
        if (userRole === 'profesor') {
            ...
            if (misCompanias && misCompanias.length > 0) {
                queryCompanias = queryCompanias.in('id', misCompanias.map((mc: any) => mc.compania_id))
            } else {
                queryCompanias = queryCompanias.eq('id', '00000000-0000-0000-0000-000000000000')
            }
        }
        ...
        if (dataCompanias) {
            const companiasConConteo = await Promise.all(dataCompanias.map(async (c: any) => {
    ...
    const abrirGestionMiembros = async (compania: Compania) => {
        ...
        if (data) setMiembrosActuales(data.map((d: any) => d.perfil_id))
    }
                            ...
                            <div className="space-y-2">
                                ...
                                {form.coordinador_id ? (
                                    <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/20 p-3 rounded-xl">
                                        ...
                                        <button type="button" onClick={() => setForm({ ...form, coordinador_id: '' })} className="p-1 text-blue-400 hover:text-white bg-blue-500/20 hover:bg-blue-500/40 rounded-lg transition-colors"><X size={14} /></button>
                                    </div>
                                ) : (
                                    <div className="relative">
                                        <Search className="absolute left-3 top-3.5 text-gray-500" size={16} />
                                        <input type="text" placeholder="Buscar profesor por nombre..." value={searchProf} onChange={e => setSearchProf(e.target.value)} className="w-full bg-[#111] border border-white/10 rounded-xl p-3 pl-10 text-white text-sm outline-none focus:border-blue-500 transition-colors" />
                                        {searchProf.length > 0 && (
                                            <div className="absolute z-20 w-full mt-2 max-h-40 overflow-y-auto bg-[#1a1a1c] border border-white/10 rounded-xl shadow-2xl custom-scrollbar flex flex-col">
                                                {profesores.filter(p => p.nombre_completo?.toLowerCase().includes(searchProf.toLowerCase())).map(p => (
