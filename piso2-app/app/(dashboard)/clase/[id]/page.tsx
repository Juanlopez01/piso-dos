...
type ClaseDetalle = {
    ...
    es_audicion: boolean // 👈 NUEVO: Detecta si es audición
}
...
export default function ClaseDetallePage() {
    ...
    const handleAddGuest = async (e: React.FormEvent) => {
        ...
        if (!clase.es_audicion && guestForm.tipo === 'pack' && !guestForm.packSeleccionadoId) {
        ...
        try {
            ...
            // 🌟 MAGIA AUDICIÓN: Bypass total de reglas de negocio
            if (clase.es_audicion) {
                guestForm.tipo = 'invitado'
                montoACobrarEnCaja = 0
                alumnoIdFinal = null // Siempre sin cuenta
                ...
                nombreInvitadoStr = !alumnoIdFinal ? `${guestForm.nombre} ${guestForm.apellido}`.trim() : null
            }
            ...
            toast.success(clase.es_audicion ? '¡Anotado en la audición!' : data.message)
            fetchData()
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white p-2 md:p-8 pb-32 animate-in fade-in">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/10 pb-6 gap-4 px-2">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            {clase?.es_audicion && (
                                <span className="bg-pink-500/20 text-pink-400 border border-pink-500/30 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest flex items-center gap-1 shadow-[0_0_10px_rgba(236,72,153,0.1)]">
                                    <Sparkles size={10} /> Audición
                                </span>
                            )}
                        </div>
                    ...
                    {clase?.estado !== 'cancelada' && (
                        <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
                            ...
                            <button onClick={() => setIsGuestOpen(true)} className={`w-full md:w-auto px-6 py-4 rounded-2xl font-black uppercase text-xs transition-all flex items-center justify-center gap-2 ${clase?.es_audicion ? 'bg-pink-500 text-white hover:bg-pink-400 shadow-[0_0_20px_rgba(236,72,153,0.3)]' : 'bg-[#D4E655] text-black hover:bg-white shadow-[0_0_20px_rgba(212,230,85,0.2)]'}`}>
                                <UserPlus size={18} /> <span>{clase?.es_audicion ? 'Anotar Participante' : 'Inscribir / Cobrar'}</span>
                            </button>
                        </div>
                    )}
                </div>
            ...
            <div className={`grid grid-cols-1 ${showFinance && !clase?.es_audicion ? 'lg:grid-cols-3' : 'lg:grid-cols-1 max-w-4xl mx-auto'} gap-8 px-2 md:px-0`}>

                {/* LISTA DE ALUMNOS */}
                <div className={showFinance && !clase?.es_audicion ? "lg:col-span-2 space-y-4" : "space-y-4"}>
                    <div className="flex justify-between items-center px-1">
                        <h3 className="text-lg font-black uppercase tracking-tighter flex items-center gap-2"><Users size={18} className="text-[#D4E655]" /> {clase?.es_audicion ? 'Participantes' : 'Alumnos'}</h3>
                            ...
                            <div key={insc.id} className={`p-4 border-b border-white/5 flex items-center justify-between gap-3 ${insc.presente ? 'bg-[#D4E655]/5' : ''}`}>
                                <div className="flex-1 min-w-0">
                                    <div className="font-bold text-white text-sm md:text-lg leading-tight truncate flex items-center gap-2">
                                        ...
                                        {insc.es_invitado && <span className={`text-[8px] text-white px-1.5 py-0.5 rounded uppercase ${clase?.es_audicion ? 'bg-pink-500' : 'bg-purple-500'}`}>{clase?.es_audicion ? 'Audición' : 'Guest'}</span>}
                                    </div>
                                    {!clase?.es_audicion && (
                ...
                {/* SIDEBAR FINANCIERO (Se oculta si es audición porque no hay plata en juego) */}
            ...
            {/* MODAL INSCRIPCIÓN / AUDICIÓN RÁPIDA */}
            {isGuestOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-in fade-in" onClick={() => setIsGuestOpen(false)}>
                    <div className={`bg-[#09090b] border ${clase?.es_audicion ? 'border-pink-500/30' : 'border-white/10'} w-full max-w-lg rounded-3xl p-6 md:p-8 shadow-2xl overflow-y-auto max-h-[90vh]`} onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-6">
                            <h3 className={`text-2xl font-black uppercase tracking-tighter ${clase?.es_audicion ? 'text-pink-500' : 'text-white'}`}>
                                {clase?.es_audicion ? 'Anotar Participante' : 'Inscripción / Cobro'}
                            </h3>
                            <button onClick={() => setIsGuestOpen(false)} className="text-gray-500 hover:text-white"><X size={24} /></button>
                        </div>

                        <form onSubmit={handleAddGuest} className="space-y-5">

                            {clase?.es_audicion ? (
                                // --- INTERFAZ EXCLUSIVA PARA AUDICIONES (Ultra rápida) ---
                                <div className="space-y-4 animate-in fade-in">

                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Nombre <span className="text-pink-500">*</span></label>
                                            <input required autoFocus value={guestForm.nombre} onChange={e => setGuestForm({ ...guestForm, nombre: e.target.value })} className="w-full bg-[#111] border border-white/10 rounded-xl p-4 text-white font-bold outline-none focus:border-pink-500 transition-colors" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">Apellido <span className="text-pink-500">*</span></label>
                                            <input required value={guestForm.apellido} onChange={e => setGuestForm({ ...guestForm, apellido: e.target.value })} className="w-full bg-[#111] border border-white/10 rounded-xl p-4 text-white font-bold outline-none focus:border-pink-500 transition-colors" />
                                        </div>
                                    </div>

                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-gray-500 uppercase ml-1">WhatsApp / Teléfono <span className="text-pink-500">*</span></label>
                                        <input required value={guestForm.telefono} onChange={e => setGuestForm({ ...guestForm, telefono: e.target.value })} className="w-full bg-[#111] border border-white/10 rounded-xl p-4 text-white outline-none focus:border-pink-500 transition-colors" placeholder="Ej: 11..." />
                                    </div>

                                    <button disabled={processing} type="submit" className="w-full bg-pink-500 text-white font-black uppercase py-5 rounded-2xl text-sm tracking-widest shadow-[0_0_20px_rgba(236,72,153,0.3)] hover:bg-pink-400 transition-all disabled:opacity-50 mt-6 flex justify-center items-center gap-2">
                                        {processing ? <Loader2 className="animate-spin" /> : <><UserPlus size={18} /> Anotar en Lista</>}
                                    </button>
                                </div>
                            ) : (
                                // --- INTERFAZ NORMAL DE COBROS Y CLASES ---
                                <>
                                    ...
                                    {!alumnoSeleccionado ? (
                                        ...
                                        <div className="bg-[#D4E655]/10 border border-[#D4E655]/30 p-4 rounded-xl flex items-center justify-between animate-in fade-in">
                                    ...
                                    <div className="space-y-2 animate-in fade-in delay-75">
