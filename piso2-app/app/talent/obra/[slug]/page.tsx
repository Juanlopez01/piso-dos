...
import { getObraBySlugAction, crearPostulacionObraAction, type ObraPublica } from '@/app/actions/talent'
import { toast, Toaster } from 'sonner'
import { Loader2, Upload, ArrowLeft, CheckCircle2, X, MapPin, CalendarDays } from 'lucide-react'
...
const RUBROS = ['Bailarin/a', 'Acrobata', 'Modelo', 'Cantante', 'Musico/a', 'Actor/Actriz', 'Otro']
...
export default function ObraPublicaPage() {
    ...
    const [form, setForm] = useState({ nombre: '', email: '', telefono: '', sexo: '', rubro: '', edad: '', altura: '', nacionalidad: '', descripcion: '' })
    ...
    useEffect(() => {
        getObraBySlugAction(params.slug as string)
            .then(d => { if (d) setObra(d); else setNotFound(true); setLoading(false) })
    ...
    const handleFotos = async (files: FileList | null) => {
        ...
        for (const file of Array.from(files)) {
            ...
            } catch (e: any) { toast.error('No se pudo subir una foto: ' + (e.message || '')) }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setEnviando(true)
        const res = await crearPostulacionObraAction({
            obraId: obra!.id, nombre: form.nombre, email: form.email, telefono: form.telefono,
            rubro: form.rubro, descripcion: form.descripcion,
            edad: form.edad ? Number(form.edad) : undefined, altura: form.altura ? Number(form.altura) : undefined,
            nacionalidad: form.nacionalidad, sexo: form.sexo || undefined,
            fotos, videos: videos.filter(v => v.trim())
        })
    ...
    if (notFound || !obra) return (
        <div className={`min-h-screen bg-white flex flex-col items-center justify-center gap-4 text-neutral-500 px-6 text-center ${sans.className}`}>
            <p className="uppercase tracking-widest text-sm font-semibold">Esta obra no existe o ya cerró.</p>
    ...
    if (listo) return (
        <div className={`min-h-screen bg-white text-neutral-900 flex flex-col items-center justify-center px-6 text-center ${sans.className}`}>
            ...
            <p className="text-neutral-500 text-sm max-w-sm leading-relaxed">Piso 2 va a revisar tu perfil para <span className="font-semibold text-neutral-700">{obra.titulo}</span>. Si quedás seleccionado/a, te contactamos.</p>
    ...
    const vencida = obra.fecha_limite && new Date(obra.fecha_limite) < new Date()
    const embed = toEmbed(obra.video_url)
    ...
    return (
        <div className={`min-h-screen bg-white text-neutral-900 ${sans.className}`}>
            ...
            <header className="pt-12 pb-6 text-center px-6">
                <p className={`${serif.className} text-[11px] tracking-[0.5em] text-neutral-500 uppercase`}>Piso 2 Talent · Obra</p>
                <h1 className={`${serif.className} text-3xl md:text-5xl tracking-[0.08em] font-medium mt-2 max-w-2xl mx-auto`}>{obra.titulo}</h1>
                <div className="flex flex-wrap items-center justify-center gap-4 mt-5 text-xs text-neutral-500">
                    {obra.ubicacion && <span className="flex items-center gap-1.5"><MapPin size={13} /> {obra.ubicacion}</span>}
                    {obra.fecha_limite && <span className="flex items-center gap-1.5"><CalendarDays size={13} /> Hasta {new Date(obra.fecha_limite + 'T12:00:00').toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>}
                </div>
            </header>

            {/* FLYER / VIDEO */}
            {(embed || obra.flyer_url || obra.video_url) && (
            ...
            {(obra.descripcion || obra.requisitos) && (
                <div className="max-w-2xl mx-auto px-6 pb-6">
                    {obra.descripcion && <p className="text-neutral-600 text-sm leading-relaxed whitespace-pre-line mb-5">{obra.descripcion}</p>}
                    {obra.requisitos && (
                        <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5">
                            <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-neutral-400 mb-2">Requisitos</p>
                            <p className="text-neutral-700 text-sm leading-relaxed whitespace-pre-line">{obra.requisitos}</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {vencida ? (
                <div className="max-w-2xl mx-auto px-6 pb-24">
                    <div className="py-16 text-center border-2 border-dashed border-neutral-200 rounded-2xl">
                        <p className="text-neutral-400 font-bold uppercase text-xs tracking-widest">El plazo para postularse ya cerró.</p>
                    </div>
                </div>
            ) : (
                <form onSubmit={handleSubmit} className="max-w-2xl mx-auto px-6 pb-24 space-y-6">
                    <div className="border-t border-neutral-200 pt-8"><h2 className={`${serif.className} text-xl md:text-2xl tracking-wide mb-6`}>Postulate</h2></div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div><label className={labelCls}>Nombre completo *</label><input required value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} className={inputCls} placeholder="Nombre y apellido" /></div>
                        <div><label className={labelCls}>Email *</label><input required type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className={inputCls} placeholder="tu@email.com" /></div>
                        ...
                        <div><label className={labelCls}>Sexo</label>
                            <select value={form.sexo} onChange={e => setForm({ ...form, sexo: e.target.value })} className={inputCls}>
                                <option value="">Elegir...</option><option value="mujeres">Femenino</option><option value="varones">Masculino</option>
                            </select>
                        </div>
                        <div><label className={labelCls}>Rubro</label>
                            <select value={form.rubro} onChange={e => setForm({ ...form, rubro: e.target.value })} className={inputCls}>
                                <option value="">Elegir...</option>{RUBROS.map(r => <option key={r} value={r}>{r}</option>)}
                            </select>
                        </div>
                        <div><label className={labelCls}>Edad</label><input type="number" min={0} value={form.edad} onChange={e => setForm({ ...form, edad: e.target.value })} className={inputCls} placeholder="Ej: 24" /></div>
                        <div><label className={labelCls}>Altura (cm)</label><input type="number" min={0} value={form.altura} onChange={e => setForm({ ...form, altura: e.target.value })} className={inputCls} placeholder="Ej: 170" /></div>
                    ...
                    <div><label className={labelCls}>Descripción</label><textarea value={form.descripcion} onChange={e => setForm({ ...form, descripcion: e.target.value })} rows={4} className={`${inputCls} resize-none`} placeholder="Contanos sobre vos, tu experiencia..." /></div>
                    ...
                    <div>
                        ...
                        <div className="space-y-2.5">
                            {videos.map((v, i) => <input key={i} type="url" value={v} onChange={e => setVideo(i, e.target.value)} className={inputCls} placeholder={i === 0 ? 'YouTube, Instagram, Vimeo...' : 'Otro link (opcional)'} />)}
                        </div>
                    </div>

                    <button type="submit" disabled={enviando || subiendo} className="w-full bg-neutral-900 text-white font-semibold uppercase tracking-[0.2em] text-xs py-4 hover:bg-black transition-colors disabled:opacity-40 flex items-center justify-center gap-2">
