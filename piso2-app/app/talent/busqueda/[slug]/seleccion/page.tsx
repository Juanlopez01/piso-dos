...
function PrintCandidato({ p }: { p: Postulante }) {
    ...
    return (
        <div style={{ breakInside: 'avoid', border: '1px solid #e5e5e5', borderRadius: '10px', padding: '12px', marginBottom: '12px' }}>
            {/* Hasta 3 fotos, una al lado de la otra, manteniendo su proporción (sin recorte) */}
            {p.fotos.length > 0 && (
                <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', marginBottom: '10px' }}>
                    {p.fotos.slice(0, 3).map((f, i) => (
                        <div key={i} style={{ flex: 1, minWidth: 0 }}>
                            <img src={f} alt={`${p.nombre} ${i + 1}`} style={{ width: '100%', height: 'auto', display: 'block', borderRadius: '6px', background: '#f5f5f5' }} />
                        </div>
                    ))}
                </div>
            )}
